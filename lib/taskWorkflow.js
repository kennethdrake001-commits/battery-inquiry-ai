...
import { getCustomerNextAction, getCustomerQueueCategory } from "./displayText";
...
function buildTaskForCustomer(customer, now) {
  ...
  const queueCategory = getCustomerQueueCategory(customer, now);

  let taskAction = "";
  let taskReason = "";

  if (queueCategory === "hidden") {
    return null;
  }
  ...
  if (queueCategory === "reply_pending") {
    ...
    taskReason = "客户已回复待处理";
  } else if (queueCategory === "quoted_follow_up") {
    taskAction = nextAction;
    taskReason = "已报价待跟进";
  } else if (queueCategory === "sent_info_follow_up") {
    taskAction = nextAction;
    taskReason = "已发资料待跟进";
  } else if (queueCategory === "today_follow_up") {
    taskAction = nextAction;
    taskReason = "今日需跟进";
  } else if (queueCategory === "overdue_follow_up") {
    taskAction = nextAction;
    taskReason = "超期未跟进";
  } else if (queueCategory === "need_quotation") {
    taskAction = nextAction;
    taskReason = "待报价";
  } else if (queueCategory === "prospecting") {
    taskAction = nextAction;
    taskReason = "主动开发待推进";
  } else if (queueCategory === "new_inquiry") {
    taskAction = nextAction;
    taskReason = "新询盘待处理";
  }

  if (!taskReason && missingInfo.includes("Order quantity") && missingInfo.includes("Destination city/country")) {
    taskAction = "Ask customer for order quantity and destination city/country";
    taskReason = "DDP 报价缺少目的城市/国家 / 缺少订单数量";
  } else if (!taskReason && missingInfo.includes("Order quantity")) {
    taskAction = "Ask customer for order quantity";
    taskReason = "缺少订单数量";
  } else if (!taskReason && missingInfo.includes("Destination city/country")) {
    taskAction = "Ask customer for destination city/country";
    taskReason = "DDP 报价缺少目的城市/国家";
  } else if (!taskReason && shippingTerm === "DDP" && hasQuantity && hasDestinationCity) {
